...
const { digest, getDigest } = useDigest();
...
//   if (type === "pdf" && digest.value.article_pdf) {
//     window.open(digest.value.article_pdf, "_blank");
//   }
// };
onMounted(() => {
  getDigest(slug);
...
<template>
  <div class="article-view" v-if="digest && digest.slug">
    <main-seacrh />
    ...
    <div class="article-view__container">
      ...
      <div class="article-view__body">
        {{ digest.description }}
        <div v-html="digest.full_description"></div>
        <div>
        ...
        <div
          ...
          v-if="digest.images && digest.images.length > 0"
        >
          <div
            class="article-view__body-image"
            v-for="img in digest.images"
            :key="img.id"
          >
    ...
    <div class="article-view__images">
      <q-uploader
        ref="imagesFiles"
        hide-upload-btn
        multiple
    ...
    <div class="article-view__documents">
      ...
      <div class="article-view__documents-list">
        ...
        <!-- <div
          @click="saveFile('pdf')"
          :style="`cursor: ${
            !digest.article_pdf ? 'not-allowed' : 'pointer'
          };`"
        >
          <img src="/src/assets/svg/pdfSvg.svg" alt="pdf" />
          <q-tooltip v-if="!digest.article_pdf">
    ...
    <div
      style="
        ...
        font-weight: 400;
      "
      v-if="Array.isArray(digest.comments)"
    >
      Оставьте свой комментарий
    </div>
    <div class="q-gutter-sm" v-if="Array.isArray(digest.comments)">
      <q-editor
    ...
    <div class="article-view__comments" v-if="Array.isArray(digest.comments)">
      <div>Все отзывы <icon-down-arrow /></div>
      <div
        class="article-view__comment comment"
        v-for="comment in digest.comments"
        :key="comment.id"
      >
    ...
    <div v-else>комментариев: {{ digest?.comments }}</div>
  </div>
